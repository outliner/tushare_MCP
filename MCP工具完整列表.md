...
本文档列出了所有支持的MCP工具，共 **53个工具**，分为10个类别。
...
### 概念板块数据
44. **get_eastmoney_concept_board**
    - 功能：获取东方财富概念板块行情数据
...
45. **get_eastmoney_concept_member**
    - 功能：获取东方财富板块每日成分数据
...
46. **get_eastmoney_concept_daily**
...
### 概念板块Alpha分析
47. **analyze_concept_alpha_strategy**
...
49. **rank_concepts_alpha_velocity**
    - 功能：概念板块Alpha排名上升速度，识别快速上升概念
    - 参数：benchmark_code（基准指数，默认000300.SH）、end_date（结束日期，YYYYMMDD格式，默认今天）
    - 说明：计算排名上升速度（当天对比前一天和前两天的排名变化），正数表示排名上升，负数表示排名下降

### 板块资金流向
50. **get_concept_moneyflow_dc**
...
### 概念板块成交量异动
51. **scan_concepts_volume_anomaly**
    - 功能：分析东财概念板块成交量异动
