...
def register_alpha_tools(mcp: "FastMCP"):
    ...
    def rank_l2_sectors_by_alpha(
        ...
        try:
            # 直接使用已知的二级行业代码列表（已发布指数的行业，is_pub=1）
            # 这样可以避免从API获取和筛选的复杂性，提高可靠性
            sector_codes = [
    ...
    def rank_l2_sectors_alpha_velocity(
        ...
        try:
            ...
            output.append("-" * 120)
            output.append(f"{'排名':<6} {'行业代码':<12} {'当天Alpha':<12} {'较昨日上升':<12} {'较前天上升':<12}")
            output.append("-" * 120)
            ...
            if not df_1d.empty:
                ...
                output.append("🚀 一天内上升位数排行（前10名）：")
                output.append("-" * 120)
                output.append(f"{'排名':<6} {'行业代码':<12} {'当天排名':<10} {'上升位数':<12} {'当天Alpha':<12}")
                output.append("-" * 120)
                ...
                for idx, (_, row) in enumerate(df_1d.head(10).iterrows(), 1):
                    ...
                    change_1d = f"{int(row['rank_change_1d']):+d}"
                    alpha = f"{row['current_alpha']*100:.2f}%" if pd.notna(row['current_alpha']) else "-"
                    output.append(f"{idx:<6} {sector_code:<12} {rank:<10} {change_1d:<12} {alpha:<12}")
            ...
            if not df_2d.empty:
                ...
                for idx, (_, row) in enumerate(df_2d.head(10).iterrows(), 1):
                    ...
                    change_2d = f"{int(row['rank_change_2d']):+d}"
                    alpha = f"{row['current_alpha']*100:.2f}%" if pd.notna(row['current_alpha']) else "-"
                    output.append(f"{idx:<6} {sector_code:<12} {rank:<10} {change_2d:<12} {alpha:<12}")
                
                output.append("")
            
            output.append("📝 说明：")
            output.append("  - Alpha = 板块收益率 - 基准收益率（沪深300）")
            output.append("  - 上升位数 = 前一日排名 - 当前排名（正数表示排名上升）")
            output.append("  - 建议关注上升位数较大的行业，可能具有较强动能")
            output.append("")
