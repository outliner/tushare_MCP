...
def register_index_tools(mcp: "FastMCP"):
    ...
    def get_index_rt_k(ts_code: str) -> str:
        ...
        try:
            pro = ts.pro_api()
            # 获取实时指数数据
            df = pro.rt_idx_k(ts_code=ts_code)
            
            if df is None or df.empty:
                return f"未找到指数 {ts_code} 的实时行情数据"
            
            # 格式化输出
            return format_index_rt_k(df)
            
        except Exception as e:
            import traceback
            error_detail = traceback.format_exc()
            return f"实时行情查询失败：{str(e)}\n{error_detail}"
...
def format_index_rt_k(df: pd.DataFrame) -> str:
    """
    格式化指数实时日线行情数据输出
    
    参数:
        df: 指数实时日线行情DataFrame
    ...
    result = []
    result.append("📊 交易所指数实时日线行情")
    result.append("=" * 110)
    result.append("")
    
    # 打印表格列名
    header = f"{'代码':<12} {'名称':<12} {'更新时间':<10} {'现价':<10} {'昨收':<10} {'涨跌额':<10} {'涨跌幅':<10} {'最高':<10} {'最低':<10} {'成交量(k)':<12} {'成交额(M)':<12}"
    ...
    for _, row in df.iterrows():
        ...
        # 计算涨跌
        change = close - pre_close if pd.notna(close) and pd.notna(pre_close) else 0
        pct_chg = (change / pre_close * 100) if pre_close != 0 else 0
        
        # 格式化显示
        close_str = f"{close:.2f}"
        pre_close_str = f"{pre_close:.2f}"
        ...
        result.append(f"{ts_code:<12} {name:<12} {trade_time:<10} {close_str:<10} {pre_close_str:<10} {change_str:<10} {pct_chg_str:<10} {high_str:<10} {low_str:<10} {vol_str:<12} {amount_str:<12}")
    
    result.append("")
    result.append(f"共统计 {len(df)} 个指数行情 snapshot")
