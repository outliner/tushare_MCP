    ...
    def _init_database(self):
        ...
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS stock_daily_data (
                ...
                amount REAL,                     -- 成交额（千元）
                num INTEGER,                     -- 成交笔数
                bid_price1 REAL,                 -- 委托买价
                bid_volume1 REAL,                -- 委托买量
                ask_price1 REAL,                 -- 委托卖价
                ask_volume1 REAL,                -- 委托卖量
        ...
        for col_def in [
            ...
            try:
                cursor.execute(f'ALTER TABLE stock_daily_data ADD COLUMN {col_def[0]} {col_def[1]}')
            except:
                pass  # 列已存在
            ...
            try:
                ...
                cursor.execute('''
                    INSERT OR REPLACE INTO stock_daily_data (
                        ...
                        bid_price1, bid_volume1, ask_price1, ask_volume1, created_at
                    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ''', (
                    str(row.get('ts_code', '')),
